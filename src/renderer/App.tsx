...
export default function App() {
  ...
  const syncWithBackend = async () => {
    try {
      ...
      // Дедупликация при синхронизации
      const uniqueLlms = deduplicate(backendLlms) as LLMConfig[]
      const uniqueRoles = deduplicate(backendRoles) as Role[]
      
      setLlms(uniqueLlms)
      setRoles(uniqueRoles)
      setKnowledgeBases(backendKbs)
      save('nob_llms', uniqueLlms)
      save('nob_roles', uniqueRoles)
      
      addLog('СИНХРОН', `LLM: ${uniqueLlms.length}, Роли: ${uniqueRoles.length}, Базы: ${backendKbs.length}`)
    } catch (e: any) {
      addLog('ОШИБКА', `Синхронизация: ${e.message}`)
    }
  }
