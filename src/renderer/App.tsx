        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.9)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#021c28', borderRadius: 20, padding: 24, maxWidth: 500, width: '100%', maxHeight: '80vh', overflow: 'auto', border: '1px solid rgba(255,255,255,0.12)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h3 style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 18 }}>{showModal === 'telegram' ? '📱 Telegram Bot' : showModal === 'web' ? '🧪 Тест AI агента' : '🌐 Web UI'}</h3>
              <button onClick={() => setShowModal(null)} style={{ background: 'none', border: 'none', color: '#fff', fontSize: 24, cursor: 'pointer' }}>✕</button>
            </div>
            
            {showModal === 'telegram' && (
              <div>
                <p style={{ opacity: 0.8, marginBottom: 16, fontSize: 14 }}>Для запуска:</p>
                <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 14, marginBottom: 14 }}>
                  <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 6 }}>Токен бота:</div>
                  <input placeholder="Введите токен" style={{ width: '100%', padding: 12, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, color: '#fff', fontSize: 13 }} />
                </div>
                <button style={{ width: '100%', padding: 14, background: '#3ddb7f', border: 'none', borderRadius: 12, color: '#000', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Запустить бота</button>
              </div>
            )}
            ...
            {showModal === 'web' && (
              <div>
                {activeRole ? (
                  <>
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 14, opacity: 0.7, marginBottom: 6 }}>🎭 Активная роль: <span style={{ color: '#3ddb7f', fontWeight: 700, fontSize: 16 }}>{activeRole.name}</span></div>
                      <div style={{ fontSize: 11, opacity: 0.5 }}>🤖 LLM: {llms.find(l => l.id === activeRole.llmId)?.name || 'не выбран'}</div>
                      <div style={{ fontSize: 11, opacity: 0.5 }}>📚 Базы знаний: {activeRole.knowledgeBases.length}</div>
                    </div>
                    
                    <div style={{ marginBottom: 12 }}>
                      <label style={{ display: 'block', fontSize: 12, opacity: 0.7, marginBottom: 6 }}>💬 Задайте вопрос:</label>
                      <textarea 
                        value={userInput} 
                        onChange={e => setUserInput(e.target.value)}
                        placeholder={`Задайте вопрос агенту "${activeRole.name}"...`}
                        rows={3}
                        style={{ width: '100%', padding: 12, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, color: '#fff', fontSize: 13, resize: 'none' }}
                      />
                    </div>
                    ...
                    <button 
                      ...
                      style={{ width: '100%', padding: 14, background: userInput.trim() ? '#3ddb7f' : 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 12, color: userInput.trim() ? '#000' : '#666', fontSize: 14, fontWeight: 600, cursor: userInput.trim() ? 'pointer' : 'default' }}
                    >
                      ▶️ Отправить запрос
                    </button>
                  </>
                  ...
                  <div style={{ textAlign: 'center', padding: 20 }}>
                    <p style={{ opacity: 0.7, marginBottom: 16 }}>Выберите роль на вкладке "Роли" и нажмите "Запустить и наблюдать"</p>
                    <button onClick={() => setShowModal(null)} style={{ padding: '10px 20px', background: '#3ddb7f', border: 'none', borderRadius: 10, color: '#000', cursor: 'pointer' }}>Закрыть</button>
                  </div>
                )}
              </div>
            )}
            ...
            {aiThinking.length > 0 && (
              <div style={{ marginTop: 24, borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 20 }}>
                <div style={{ fontWeight: 600, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>🧠 Мониторинг AI <span style={{ background: 'rgba(61,219,127,0.2)', color: '#3ddb7f', padding: '2px 8px', borderRadius: 10, fontSize: 10 }}>LIVE</span></div>
                <div style={{ maxHeight: 350, overflow: 'auto' }}>
                  {aiThinking.map((item, i) => (
                    <div key={i} style={{ 
                      background: item.status === 'Готово' ? 'rgba(61,219,127,0.1)' : 'rgba(0,0,0,0.3)', 
                      borderRadius: 10, 
                      padding: 14, 
                      marginBottom: 10, 
                      borderLeft: `4px solid ${item.status === 'Готово' ? '#3ddb7f' : item.status === 'Работает' ? '#21b9f4' : '#ff8c42'}`,
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        ...
                        <span style={{ 
                          ...
                          padding: '2px 8px', 
                          borderRadius: 10, 
                          background: item.status === 'Готово' ? 'rgba(61,219,127,0.3)' : item.status === 'Работает' ? 'rgba(33,185,244,0.3)' : 'rgba(255,140,66,0.3)',
                          color: item.status === 'Готово' ? '#3ddb7f' : item.status === 'Работает' ? '#21b9f4' : '#ff8c42'
                        }}>
                          {item.status}
                        </span>
                      </div>
                      {item.input && (
                        <div style={{ marginBottom: 6 }}>
                          <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 2 }}>📥 ВХОД:</div>
                          <div style={{ fontSize: 12, color: '#fff', padding: '6px 10px', background: 'rgba(255,255,255,0.05)', borderRadius: 6 }}>{item.input}</div>
                        </div>
                      )}
                      {item.output && (
                        <div>
                          <div style={{ fontSize: 10, opacity: 0.5, marginBottom: 2 }}>📤 ВЫХОД:</div>
                          <div style={{ fontSize: 12, color: '#3ddb7f', padding: '8px 10px', background: 'rgba(61,219,127,0.1)', borderRadius: 6, whiteSpace: 'pre-wrap' }}>{item.output}</div>
                        </div>
                      )}
                    </div>
                  ))}
